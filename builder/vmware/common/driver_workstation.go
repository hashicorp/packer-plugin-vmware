...
func (d *WorkstationDriver) Verify() error {
	...
	if d.VdiskManagerPath == "" {
		d.VdiskManagerPath, err = workstationFindVdiskManager()
	}

	if err != nil {
		return fmt.Errorf("error finding either %s: %s", appVdiskManager, err)
	}

	if path := d.VdiskManagerPath; path != "" {
		if _, err := os.Stat(path); err != nil {
			if os.IsNotExist(err) {
				return fmt.Errorf("%s not found at: %s", appVdiskManager, path)
			}
			return err
		}
		log.Printf("[INFO] - %s found at: %s", appVdiskManager, path)
	}
