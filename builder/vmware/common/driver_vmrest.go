...
// VMRest driver talks to an ESXi5 hypervisor remotely over SSH to build
...
func (d *VMRestDriver) Clone(dst, src string, linked bool, snapshot string) error {
...
func (d *VMRestDriver) CompactDisk(diskPathLocal string) error {
	diskPath := d.datastorePath(diskPathLocal)
	return d.sh("vmkfstools", "--punchzero", strconv.Quote(diskPath))
}

func (d *VMRestDriver) CreateDisk(diskPathLocal string, size string, adapter_type string, typeId string) error {
	diskPath := strconv.Quote(d.datastorePath(diskPathLocal))
	return d.sh("vmkfstools", "-c", size, "-d", typeId, "-a", adapter_type, diskPath)
}

func (d *VMRestDriver) CreateSnapshot(vmxPath string, snapshotName string) error {
	_, err := d.run(nil, "vim-cmd", "vmsvc/snapshot.create", d.vmId, snapshotName)
	return err
}

func (d *VMRestDriver) IsRunning(string) (bool, error) {
...
func (d *VMRestDriver) Start(vmxPathLocal string, headless bool) error {
	for i := 0; i < 20; i++ {
...
func (d *VMRestDriver) Stop(vmxPathLocal string) error {
	return d.sh("vim-cmd", "vmsvc/power.off", d.vmId)
}

func (d *VMRestDriver) Register(vmxPathLocal string) error {
...
func (d *VMRestDriver) SuppressMessages(vmxPath string) error {
	return nil
}

func (d *VMRestDriver) Unregister(vmxPathLocal string) error {
...
func (d *VMRestDriver) UploadISO(localPath string, checksum string, ui packersdk.Ui) (string, error) {
...
func (d *VMRestDriver) VerifyOvfTool(SkipExport, skipValidateCredentials bool) error {
...
func (d *VMRestDriver) PotentialGuestIP(multistep.StateBag) ([]string, error) {
...
func (d *VMRestDriver) GuestAddress(multistep.StateBag) (string, error) {
...
func (d *VMRestDriver) VNCAddress(ctx context.Context, _ string, portMin, portMax int) (string, int, error) {
	var vncPort int
...
func (VMRestDriver) UpdateVMX(_, password string, port int, data map[string]string) {
...
func (d *VMRestDriver) Remove(path string) error {
...
func (d *VMRestDriver) SetOutputDir(path string) {
...
func (d *VMRestDriver) checkSystemVersion() error {
...
func (d *VMRestDriver) checkGuestIPHackEnabled() error {
...
func (d *VMRestDriver) mkdir(path string) error {
...
func (d *VMRestDriver) Export(args []string) error {
	return d.base.Export(args)
}

// VerifyChecksum checks that file on the esxi instance matches hash
func (d *VMRestDriver) VerifyChecksum(hash string, file string) bool {
	if hash == "none" {
...
func (d *VMRestDriver) ssh(command string, stdin io.Reader) (*bytes.Buffer, error) {
	ctx := context.TODO()
...
func (d *VMRestDriver) run(stdin io.Reader, args ...string) (string, error) {
...
func (d *VMRestDriver) sh(args ...string) error {
	_, err := d.run(nil, args...)
	return err
}

func (d *VMRestDriver) esxcli(args ...string) (*esxcliReader, error) {
...
func (d *VMRestDriver) AcquireVNCOverWebsocketTicket() (*types.VirtualMachineTicket, error) {
