...
func (c *ToolsConfig) Prepare(ctx *interpolate.Context) []error {
	...
	if c.ToolsMode == toolsModeAttach && c.ToolsUploadFlavor != "" && c.ToolsSourcePath != "" {
		errs = append(errs, errors.New("'tools_upload_flavor' and 'tools_source_path' cannot both be specified with 'tools_mode=\"attach\"'"))
		return errs
	}
