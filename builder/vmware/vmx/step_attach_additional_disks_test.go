...
func TestRun_AttachAdditionalDisks_SkipsReservedUnit7(t *testing.T) {
	...
	cfg := &Config{}
	cfg.VMName = "testvm"
	cfg.DiskAdapterType = "" // default to scsi
	cfg.DiskTypeId = ""
	...
	if got := updated["scsi0:8.filename"]; got != "testvm-1.vmdk" {
		t.Fatalf("expected scsi0:8.filename testvm-1.vmdk, got %q", got)
	}
	...
	if got := updated["scsi0:9.filename"]; got != "testvm-2.vmdk" {
		t.Fatalf("expected scsi0:9.filename testvm-2.vmdk, got %q", got)
	}
