...
**Apple Silicon-based Macs (`aarch64`)**

```shell
packer init .
packer build -var-file=pkrvars/debian/fusion-aarch64.pkrvars.hcl .
```
  ...
  > }
  > 
  > Please refer to the exanple `fusion-aarch64.pkrvars.hcl` for complete details. 
...
```shell
packer init .
packer build -var-file=pkrvars/debian/fusion-x86_64.pkrvars.hcl .
```
...
```shell
packer init .
packer build -var-file=pkrvars/debian/workstation-x86_64.pkrvars.hcl .
```